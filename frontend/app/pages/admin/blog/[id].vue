<template>
  <div class="admin-blog-edit">
    <PageHeader title="Edit Blog Post" subtitle="Admin Panel" />
            ...
            <div class="form-card">
              <form @submit.prevent="handleSubmit" class="blog-form">
                ...
                <div class="mb-4">
                  ...
                  <small class="form-text">Toggle to publish or unpublish this post.</small>
                </div>
                ...
                <div class="form-actions">
                  ...
                  <NuxtLink :to="`/blog/${post.slug}`" class="btn btn-info" target="_blank">
...
const { fetchBlogPostById, updateBlogPost } = useBlogApi()
...
// Form data
const formData = reactive<BlogPostUpdate>({
  heading: '',
  content: '',
  author: '',
  feature_image: '',
...
const loadPost = async () => {
  ...
  try {
    ...
    if (response) {
      post.value = response
      // Populate form
      formData.heading = response.heading
      formData.content = response.content
      formData.author = response.author
      formData.feature_image = response.feature_image
      formData.published = response.published
    }
  } catch (err: unknown) {
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (result) {
      ...
      // Reload the post to get updated timestamp
      await loadPost()
...
useHead({
  title: post.value ? `Edit: ${post.value.heading} - Admin` : 'Edit Post - Admin'
})
</script>
